...
`)
  ...
  assert.match(readlinkSync(candidate), /0\.3\.0-phase7\.7$/)
  assert.equal(existsSync(join(data, 'current')), false)
  assert.equal(
    JSON.parse(readFileSync(join(
      data,
      'releases/0.3.0-phase7.7/package.json',
    ))).version,
    '0.3.0-phase7.7',
  )
  ...
  assert.match(
    unit,
    /releases\/0\.3\.0-phase7\.7\/src\/main\.mjs/,
  )
  assert.match(
    unit,
    /MARVEEN_CODEX_BRIDGE_RUNTIME_MODULE=.*releases\/0\.3\.0-phase7\.7\/src\/codex-runtime-module\.mjs/,
  )
  assert.match(
    unit,
    /MARVEEN_CODEX_BRIDGE_BETTER_SQLITE3_PATH=.*releases\/0\.3\.0-phase7\.7\/node_modules\/better-sqlite3/,
  )
