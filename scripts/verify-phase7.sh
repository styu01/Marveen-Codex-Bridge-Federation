...
NODE_OPTIONS=--no-warnings "${NODE_BIN}" -e '
  ...
  if (pkg.version !== "0.3.0-phase7.7") throw new Error("wrong package version")
...
grep -Eq '^(#|ℹ) tests 113$' "${TEST_LOG}" \
  || fail "expected exactly 113 tests"
grep -Eq '^(#|ℹ) pass 113$' "${TEST_LOG}" \
  || fail "expected exactly 113 passing tests"
...
pass "all 113 Phase 1-7 mock, security, installer, cutover and rollback tests pass with no skip"
