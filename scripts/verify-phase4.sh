...
usage() {
  cat <<'EOF'
Usage:
  verify-phase4.sh [--node-bin PATH] [--better-sqlite3-path PATH]
...
service. The default real preflight starts only a disposable Codex App Server
child process. It does not patch Marveen and does not install, stop, restart,
enable or disable any system service. Use --mock-only only for package CI.
EOF
}
...
[[ "${NODE_VERSION}" == v22.* ]] \
  || fail "Phase 4 requires Node 22, found ${NODE_VERSION}"
...
for file in "${required_files[@]}"; do
  [[ -f "${SOURCE_ROOT}/${file}" ]] \
    || fail "required Phase 4 file is missing: ${file}"
done
pass "Phase 4 App Server runtime and Federation service surface is complete"
...
NODE_OPTIONS=--no-warnings "${NODE_BIN}" -e '
  ...
  if (pkg.version !== "0.3.0-phase4.1") throw new Error("wrong package version")
...
grep -Eq '^(#|ℹ) tests 74$' "${TEST_LOG}" \
  || fail "expected exactly 74 tests"
grep -Eq '^(#|ℹ) pass 74$' "${TEST_LOG}" \
  || fail "expected exactly 74 passing tests"
...
pass "all 74 Phase 1-4 mock and integration tests pass with no skip"

if [[ "${MOCK_ONLY}" -eq 1 ]]; then
  echo "RESULT: PHASE 4 MOCK PROTOCOL PASS (REAL CODEX NOT RUN)"
  exit 0
fi

[[ -x "${CODEX_BIN}" ]] || fail "Codex binary is missing or not executable: ${CODEX_BIN}"
if [[ -z "${REAL_OUTPUT_ROOT}" ]]; then
  REAL_OUTPUT_ROOT="${HOME}/bela-codex-preflight/phase4-real-$(date +%Y%m%d-%H%M%S)"
fi
...
echo "RESULT: PHASE 4 REAL CODEX APP SERVER PASS"
